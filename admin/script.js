...
function loadDepartmentComplaints(departmentId, departmentName) {
    ...
    fetch(`/api/admin/complaints?department=${departmentName}`, {
        credentials: 'include'
    })
        .then(response => response.json())
        .then(data => {
...
async function filterComplaints() {
    ...
    try {
        ...
        const response = await fetch(`/api/admin/complaints?department=${encodeURIComponent(department)}&status=${encodeURIComponent(status)}&search=${encodeURIComponent(search)}`, {
            credentials: 'include'
        });
        ...
        if (data.success) {
            ...
            if (imageContainer && modalImage) {
                if (complaint.image_path) {
                    modalImage.src = `/uploads/${complaint.ticket_number}.jpg`;
